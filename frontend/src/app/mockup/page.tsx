...
export default function QuestionMenu() {
  ...
  const [conversation, setConversation] = useState([
    {
      role: "ai",
      content:
        "Hello! Feel free to ask me questions as you work through the exercise.",
    },
  ]);
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      console.log("The user has the token", user.access_token);
      if (!user) {
        console.error("No user found, unable to fetch questions.");
        return;
      }
      ...
      if (!email) {
        console.error("No email found, unable to fetch questions.");
        return;
      }

      try {
        const response = await fetch(
          `http://127.0.0.1:8000/api/questions/?email=${email}`,
          {
            method: "GET",
            headers: {
        ...
        if (response.ok) {
          ...
          console.log("Fetched questions with threads:", data);
          setModules(data);
          ...
          data.forEach((module) => {
            initialAnswered[module.module] = new Set(
              module.questions
                .map((q, index) =>
                  q.answer_status === "Answered" ? index : null
                )
        ...
        console.error("Error fetching questions with threads:", error);
      }
    };
  ...
  useEffect(() => {
    if (modules.length > 0) {
      ...
      const total = currentModule?.questions.length || 0;
      setProgress((answeredCount / total) * 100);
      const allQuestionsAnswered = modules.every((module) =>
        module.questions.every(
          (question) => question.answer_status === "Answered"
        )
      );
  ...
  const callCompletionAPI = async () => {
    try {
      const response = await fetch("http://127.0.0.1:8000/api/completion/", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (response.ok) {
        const data = await response.json();
        console.log("Completion API called successfully:", data);
      } else {
        console.error("Failed to call completion API:", response.statusText);
      }
    } catch (error) {
  ...
  const handleScreenshotCaptured = (image: string | null) => {
    if (image) {
      console.log("Screenshot captured in QuestionMenu:", image);
      const currentModule = modules[currentModuleIndex];
      setScreenshot(image);
      currentQuestion.image = image;
      currentModule.questions[currentQuestionIndex];
  ...
  const handleUserReply = async () => {
    ...
    try {
      // Make the API call to get_feedback
